...
test "create IPv4Layer from slice" {
    var data = [_]u8{
    ...
    var ipv4_layer = try IPv4.IPv4Layer.initFromSlice(data[0..], allocator);

    defer ipv4_layer.deinit();

    const ipv4_header: *const IPv4.IPv4Header = ipv4_layer.get_immutable_header();

    const hdr_len = ipv4_header.get_ihl() * 4;

    try expect(hdr_len == 20);
}
