...
test "generic record" {
    ...
    var dns_layer: DNS.DNSLayer = try .init(allocator);
    defer dns_layer.deinit();
...
test "dns build" {
    ...
    var dns_layer: DNS.DNSLayer = try DNS.DNSLayer.init(allocator);
    defer dns_layer.deinit();
...
test "parse dns query raw" {
    ...
    var dns_layer = try DNS.DNSLayer.initFromSlice(ziggit_dev_a_q[0..], allocator);
    defer dns_layer.deinit();

    const dns_header: *const DNS.DNSHeader = dns_layer.get_immutable_header();
    ...
    try expect(dns_layer.get_immutable_header().get_qdcount() == 0);
...
test "parse dns A response raw" {
    ...
    var dns_layer = try DNS.DNSLayer.initFromSlice(google_a_resp[0..], allocator);
    defer dns_layer.deinit();
...
test "parse dns AAAA response raw" {
    ...
    var dns_layer = try DNS.DNSLayer.initFromSlice(cloudflare_aaaa_resp[0..], allocator);
    defer dns_layer.deinit();
...
test "parse ebay CNAME response" {
    ...
    var dns_layer = try DNS.DNSLayer.initFromSlice(ebay_cname_resp[0..], allocator);
    defer dns_layer.deinit();
...
test "parse dns txt record response" {
    ...
    var dns_layer = try DNS.DNSLayer.initFromSlice(random_org_txt_resp[0..], allocator);
    defer dns_layer.deinit();
...
test "parse MX record response" {
    var google_mx_resp = [_]u8{ 0x3a, 0x98, 0x81, 0x80, 0x0, 0x1, 0x0, 0x1, 0x0, 0x0, 0x0, 0x1, 0x6, 0x67, 0x6f, 0x6f, 0x67, 0x6c, 0x65, 0x3, 0x63, 0x6f, 0x6d, 0x0, 0x0, 0xf, 0x0, 0x1, 0xc0, 0xc, 0x0, 0xf, 0x0, 0x1, 0x0, 0x0, 0x1, 0x2c, 0x0, 0x9, 0x0, 0xa, 0x4, 0x73, 0x6d, 0x74, 0x70, 0xc0, 0xc, 0x0, 0x0, 0x29, 0x2, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0 };
    ...
    var dns_layer = try DNS.DNSLayer.initFromSlice(google_mx_resp[0..], allocator);
    defer dns_layer.deinit();
...
test "parse PTR record response" {
    ...
    var dns_layer = try DNS.DNSLayer.initFromSlice(google_ip_ptr_resp[0..], allocator);
    defer dns_layer.deinit();
