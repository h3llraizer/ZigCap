...
test "parse dns response layer" {
    ...
    const dns_buf = try allocator.alloc(u8, dns_a_resp.len);
...
test "parse dns response" {
    ...
    const dns_buf = try allocator.alloc(u8, dns_ns_response.len);
...
test "parse cname response" {
    ...
    const dns_buf = try allocator.alloc(u8, dns_cname_response.len);
...
test "parse https w ar response" {
    ...
    const dns_buf = try allocator.alloc(u8, dns_https_aa_resp.len);
