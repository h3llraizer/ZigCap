...
const LinkLayerProtocols = zigcap.ProtocolHelpers.LinkLayerProtocols;
const Layer = zigcap.Packet.Layer;
const LayerOwner = zigcap.Layer.LayerOwner;
const LayerProtocols = zigcap.ProtocolHelpers.LayerProtocols;
...
test "remove_eth" {
    ...
    const raw_data = RawData{ .mutable = try allocator.alloc(u8, udp_raw.len) };

    std.mem.copyForwards(u8, raw_data.mutable, udp_raw[0..udp_raw.len]);

    print("original ({}) {x}\n", .{ raw_data.get_len(), raw_data.get_immutable() });

    var packet = try Packet.create(allocator);
    ...
    const layer: *Layer = try packet.search_layers(LayerProtocols{ .Transport = .UDP }) orelse {
