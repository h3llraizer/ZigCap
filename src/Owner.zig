...
pub const LayerOwner = union(enum) {
    ...
    pub fn extend_layer(self: *LayerOwner, offset: usize, extend_len: usize) (LayerError || Allocator.Error)![]u8 {
        var buf: []u8 = undefined;
        switch (self.*) {
            .packet_layer => |layer| {
                buf = try layer.packet.extend_layer(&layer.layer_iface, offset, extend_len); // TODO: extend at offset instead
            },
            .owned_buffer => |*buffer| {
    ...
    pub fn shorten_layer(self: *LayerOwner, offset: usize, shorten_len: usize) Allocator.Error!void {
        switch (self.*) {
            .packet_layer => |layer| {
                try layer.packet.shorten_layer(&layer.layer_iface, offset, shorten_len);
            },
            .owned_buffer => |*buffer| {
...
pub const TLVOwner = union(enum) {
    ...
    pub fn extend_buffer(self: *TLVOwner, offset: usize, extend_len: usize) (LayerError || Allocator.Error)![]u8 {
        var buf: []u8 = undefined;
        switch (self.*) {
            .layer => |layer| {
                buf = try layer.extend_layer(offset, extend_len); // TODO: extend at offset instead
            },
            .owned_buffer => |*buffer| {
                buf = try buffer.extend(offset, extend_len);
            },
    ...
    pub fn shorten_buffer(self: *TLVOwner, offset: usize, shorten_len: usize) Allocator.Error!void {
        switch (self.*) {
            .layer => |layer| {
                try layer.shorten_layer(offset, shorten_len);
            },
            .owned_buffer => |*buffer| {
                try buffer.shorten(offset, shorten_len);
            },
